...
export const SpecialWorkManageView = ({ active, initialReportId, onReportDeleted }: { active: boolean; initialReportId?: string | null; onReportDeleted?: () => void }) => {
    ...
    const [viewMode, setViewMode] = useState<'list' | 'pivot' | 'attendance'>('pivot');
    ...
    // Auto-open if initialReportId changes or is provided
    useEffect(() => {
        if (initialReportId) {
    ...
    const handleGeneratePreview = async () => {
        if (!selectedId || !user?.company_id) {
            if (!user?.company_id) await showAlert("로그인 정보가 올바르지 않습니다.", { type: 'error' });
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                await showAlert("생성 실패: " + data.message, { type: 'error' });
            }
        } catch (e) {
            console.error(e);
    ...
    const handleApplyAttendance = async () => {
        ...
        try {
            ...
            const data = await res.json() as any;
            if (data.success) {
                await showAlert("적용되었습니다.", { type: 'success' });
                setShowPreview(false);
            } else {
                await showAlert("저장 실패: " + data.message, { type: 'error' });
            }
        } catch (e) {
            console.error(e);
            await showAlert("오류 발생", { type: 'error' });
    ...
    const pivotData = React.useMemo(() => {
        ...
        const rows = filteredRecords.map((rec: any) => {
            ...
            return {
                ...
                totalHours: rec.special_hourly_wage > 0
                    ? Math.floor(rec.total_amount / rec.special_hourly_wage)
                    : rec.calculated_hours // Fallback to DB value if wage is missing
            };
        });
    ...
    const handleDelete = async (id: string) => {
        const confirmed = await showConfirm("정말 삭제하시겠습니까? 관련 데이터가 모두 삭제됩니다.", { title: '삭제 확인', type: 'warning' });
        if (!confirmed) return;
        try {
            const res = await fetch(`/api/special-work/reports?id=${id}`, { method: 'DELETE' });
            if (res.ok) {
                await showAlert("삭제되었습니다.", { type: 'success' });
                // If we are in detail view (selectedId matches), call onReportDeleted
                if (id === selectedId && onReportDeleted) {
                    onReportDeleted();
                } else {
                    fetchReports(); // Fallback for list view
                }
            } else {
    ...
    // New handler for Detail View Delete Button
    ...
    // [UX Fix] If selectedId exists (Detail Mode), show ONLY the detail view content directly.
    // This removes the "intermediate" list modal when opening from calendar.
    if (selectedId) {
        // Find the report object for title if needed, or fetch it.
        // Wait, the Dialog below relies on `selectedId` state. 
        // We can just render the Dialog's *content* as the main view here?
        // No, the parent (SpecialWorkPage) puts this in a Dialog. 
        // So we should just render the "Detail Content" here, not a nested Dialog.

        return (
            <div className="flex flex-col h-full bg-white">
                {/* Header / Actions */}
                <div className="px-6 py-4 border-b border-slate-100 bg-white z-20 flex flex-row items-center justify-between">
                    <div className="flex items-center gap-2 text-xl">
                        <span className="text-slate-400 font-normal">상세 내역:</span>
                        <span className="text-slate-800 font-bold decoration-orange-200 underline decoration-4 underline-offset-4">
                            {detailReport?.title || '데이터 로딩 중...'}
                        </span>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-between gap-4 px-6 py-3 border-b border-slate-200 bg-white/80 backdrop-blur-sm z-10 sticky top-0">
                            ...
                            <div className="flex bg-slate-100/80 p-1.5 rounded-xl border border-slate-200 shadow-sm gap-1">
                                ...
                                <Button
                                    ...
                                    className="h-9 text-xs font-bold text-red-600 hover:bg-red-50 hover:text-red-700 hover:shadow-sm rounded-lg border border-transparent hover:border-red-100 px-3"
                                >
